...
def deal_conv_dict_block(conv_dict, bn_weight_cache, name=''):
...
def deal_conv_dict_v1(conv_dict, name=''):
    ...
    weightcache = np.concatenate((weight, bias, [1.0, 0.0, 0.0, 1-1e-5]))
    # weightcache = np.ones(shape=(weightcache.shape[0]), dtype=np.float)
    weightcache = [float(str(i)) for i in list(weightcache)]
    print(len(weightcache))
    return Conv2dLayer(name=name, weight_shape=shape, kernal_size=(1, 1), weight_cache=weightcache, filters=shape[-1])


def deal_bn_dict(conv_dict):
...
def deal_conv_block(block, name='', bn=True, act_type='leky'):
    ...
    if bn is True:
        ...
        for crr_layer in range(conv_layer_num):
            ...
            model_lst.append([LayerTypes['conv'], deal_conv_dict_block(conv, deal_bn_dict(bn_1), '%s_%s' % (name, crr_layer))])
...
LayerTypes = {'input': "NNPP.InputLayer", 'conv': "NNPP.Conv2D", 'bn': "NNPP.BatchNormalization",
              ...
              'avgpool':'AvgPooling2D', 'up': 'NNPP.UpSampling2D'}

model_name = 'net_params_1691_2019-08-28_10-07.pkl'
...
for i in range(1, 5):
    ...
    deal_conv_block(up, name='%s' % (i+4))

out = {k: v for k, v in pretrained_dict.items() if 'out' in k}
model_lst.append([LayerTypes['conv'], deal_conv_dict_v1(out, name='output_conv')])
...
f = open('Checkpoints\\test\\model_4_128.json', mode='w', encoding='utf-8')
